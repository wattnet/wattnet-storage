...
**Full tox matrix** (format check, lint, type-check, security, unit + integration):

```bash
tox
```

To apply formatting in one step instead of checking:
...
1. Implement `BaseStorageClient` in your package:
    ...
    class MyBackendClient(BaseStorageClient):
        ...
        def __init__(self, config: StorageConfig):
            plugin_cfg = config.plugin_configs.get("mybackend", {})
            # initialise your client using plugin_cfg or environment variables
...
3. Activate it by passing the backend name in `StorageConfig`:
    ...
    config = StorageConfig(
        ...
        plugin_configs={"mybackend": {"host": "myhost"}},
    )
